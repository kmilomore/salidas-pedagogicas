  ...
  return (
    <section className="rounded-[24px] border border-slate-200 bg-slate-50 p-5">
      ...
      <div className="mt-4 space-y-4 xl:hidden">
        {rows.length ? (
          rows.map((row) => (
            <article key={`${row.rbd}-${title}-mobile`} className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-semibold text-slate-950">{row.schoolName}</p>
                  <p className="text-sm text-slate-500">RBD {row.rbd}</p>
                </div>
                <p className="text-sm font-semibold text-slate-700">{row.tripCount} salida(s)</p>
              </div>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Directora(o)</p>
                  <p className="mt-1 text-sm text-slate-900">{row.directorName || "Sin nombre disponible"}</p>
                </div>
                <div className="sm:col-span-2">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Correo(s)</p>
                  <p className="mt-1 break-words text-sm text-slate-900">{row.emails.length ? row.emails.join(", ") : "Sin correo disponible"}</p>
                </div>
              </div>
          ...
          <div className="portal-table__empty rounded-[24px] border border-slate-200 bg-white">{emptyMessage}</div>
        )}
      </div>

      <div className="portal-table mt-4 hidden xl:block">
